...
/** Calculate days elapsed since a timestamp, minimum 1. */
export function daysSince(timestamp: number): number {
  return Math.max(1, Math.ceil((Date.now() - timestamp) / DAY));
}

/** Format days between creation and ship date as human-readable text. */
...
/** Format elapsed time from a timestamp as "Xm ago", "Xh ago", "Xd ago", etc. */
